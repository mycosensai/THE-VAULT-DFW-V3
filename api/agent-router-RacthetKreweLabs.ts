...
import { AGENT_DEFINITIONS, REQUIRED_AGENT_IDS, toAgentProject } from "./agent-definitions";
...
function hasOpenAIKey() {
  return Boolean(env.openaiApiKey && env.openaiApiKey.length >= 20);
}

async function ensureAgentRuntime(db: ReturnType<typeof getDb>) {
  ...
  const [provider] = await db
    .select()
    .from(agentProviders)
    .where(eq(agentProviders.providerId, "openai"))
    .limit(1);
  ...
  if (provider) {
    await db
      ...
      .set({
        ...
        kind: "openai",
        model: "gpt-4o",
        apiKey: openAIConfigured ? "env:OPENAI_API_KEY" : null,
  ...
  for (const project of AGENT_DEFINITIONS.map(toAgentProject)) {
    const [existing] = await db
      .select()
      .from(agentProjects)
      .where(eq(agentProjects.projectId, project.projectId))
      .limit(1);
    ...
    if (existing) {
      await db
        ...
        .where(eq(agentProjects.projectId, project.projectId));
      touched.push(project.projectId);
    } else {
      await db.insert(agentProjects).values(project);
    ...
    if (!fleetRow) {
      await db.insert(agentFleetState).values({ projectId: project.projectId });
    }
  }

  const promptCheck = REQUIRED_AGENT_IDS.map((projectId) => {
    const prompt = getPrompt(projectId);
    return {
      projectId,
      hasZeroHallucinationContract: prompt.includes("ZERO-HALLUCINATION CONTRACT"),
      hasPreflight: prompt.includes("PRE-FLIGHT"),
      promptLength: prompt.length,
    };
  });
  const promptFailures = promptCheck.filter((item) => !item.hasZeroHallucinationContract || !item.hasPreflight);
  const alwaysOnValue = JSON.stringify({
    enabled: true,
    requiredAgents: REQUIRED_AGENT_IDS,
    provider: "openai",
    openAIConfigured,
    checkedAt: new Date().toISOString(),
  });

  const [alwaysOnSetting] = await db
    .select()
    .from(systemSettings)
    .where(eq(systemSettings.key, "agent_fleet_always_on"))
    .limit(1);

  if (alwaysOnSetting) {
    await db
      .update(systemSettings)
      .set({ value: alwaysOnValue, updatedAt: new Date() })
      .where(eq(systemSettings.key, "agent_fleet_always_on"));
  } else {
    await db.insert(systemSettings).values({
      key: "agent_fleet_always_on",
      value: alwaysOnValue,
    });
  }

  await db.insert(agentLogs).values({
    event: "agent_fleet_always_on_verified",
    projectId: "fleet",
    data: JSON.stringify({
      requiredAgents: REQUIRED_AGENT_IDS.length,
      activeAgents: REQUIRED_AGENT_IDS.length,
      provider: "openai",
      openAIConfigured,
      promptFailures,
      touched,
  ...
  await db.insert(auditLogs).values({
    auditId: genAuditId("agent-fleet"),
    agentName: "runtime_guard",
    checkType: "hallucination",
    severity: promptFailures.length || !openAIConfigured ? "warning" : "info",
    finding: promptFailures.length
      ? "One or more agents are missing hallucination prompt protections"
      : "Zero-hallucination manifesto verified across all 10 agents",
    details: JSON.stringify({ openAIConfigured, promptCheck }),
    autoFixed: promptFailures.length === 0,
    fixApplied: "Seeded/updated all agent prompts from canonical definitions and marked all agents active",
    fixResult: openAIConfigured ? "OpenAI provider is configured through OPENAI_API_KEY" : "OPENAI_API_KEY is missing or too short",
    requiresHumanReview: !openAIConfigured || promptFailures.length > 0,
  });
...
export const agentRouter = createRouter({
  ...
  seedProjects: adminQuery.mutation(async () => {
    ...
    for (const p of AGENT_DEFINITIONS.map(toAgentProject)) {
      const [existing] = await db
        .select()
        .from(agentProjects)
    ...
    return { seeded: AGENT_DEFINITIONS.length, success: true };
  }),

  // ── ENSURE ALL 10 AGENTS ARE ACTIVE + GUARDED ──
  ensureAlwaysOn: adminQuery.mutation(async () => {
    const db = getDb();
    return ensureAgentRuntime(db);
  }),
