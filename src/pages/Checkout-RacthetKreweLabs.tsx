...
export default function Checkout() {
  ...
  const listingId = Number.parseInt(id || '0', 10)
  ...
  const { data: listing, isLoading } = trpc.listings.getById.useQuery(
    { id: listingId },
    { enabled: hasValidListingId }
  )
  const { data: verifiedSession, isLoading: isVerifying } = trpc.stripe.verifySession.useQuery(
    { sessionId },
  ...
  const handleCheckout = () => {
    ...
    createSession.mutate({
      listingId,
      successUrl: `${origin}/checkout/${listingId}?session_id={CHECKOUT_SESSION_ID}`,
  ...
  if (!hasValidListingId) {
    return (
      <div className="min-h-screen pt-32 px-4 text-center">
        <AlertCircle className="w-12 h-12 text-[#C9A84C]/50 mx-auto mb-4" />
        <h2 className="font-cinzel text-lg text-[#C8BC98] tracking-[3px] uppercase mb-2">Choose an Item First</h2>
        <p className="text-xs text-[#8A6E2F] mb-5">Checkout starts from a live marketplace listing.</p>
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        {sessionId && (
          <div className="mb-6 p-4 border border-emerald-500/25 bg-emerald-500/8 text-center">
            <div className="flex items-center justify-center gap-2 mb-1">
              ...
              <p className="font-cinzel text-[10px] tracking-[3px] uppercase text-emerald-400">
                {isVerifying ? 'Verifying Stripe Payment' : verifiedSession?.status === 'paid' ? 'Payment Verified' : 'Payment Pending'}
              </p>
            </div>
            <p className="text-[10px] text-[#C8BC98]">
              {verifiedSession?.status === 'paid'
                ? 'Stripe confirmed the payment and this item has been marked sold.'
                : 'Stripe returned you to The Vault. If payment is still processing, refresh this page in a moment.'}
            </p>
          </div>
        )}
